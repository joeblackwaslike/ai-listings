...
**No `SECURITY DEFINER`, no key restriction inside the function** — matches the repo's only other RPC (`generate_sku`), and the route's existing `EDITABLE_KEYS` allow-list remains the sole gate on which keys a client can patch. RLS's `owner_access` policy on `listings` (`0002_auth_user_isolation_api_keys.sql:10`) still protects against a client calling this RPC directly with a spoofed `p_user_id`, since the function runs as the invoking role (no `SECURITY DEFINER`) and RLS filters by `auth.uid()` regardless of what the caller passes as `p_user_id`.

**Revised during PR review (migration `0019`):** `GRANT`/`REVOKE` were added after all, in a follow-up migration — `EXECUTE` is now restricted to `service_role` only (revoked from `PUBLIC`, `anon`, `authenticated`), matching how the RPC is actually invoked today (exclusively via `getSupabaseAdmin()`'s service-role client). This is a narrower posture than `generate_sku`'s, deliberately: `generate_sku` returns a formatted string with no injectable shape, while `merge_listing_measurements` accepts an arbitrary JSONB patch, so a direct `/rpc/` call from `anon`/`authenticated` would have bypassed the route's `EDITABLE_KEYS`/positive-number validation (RLS still would have blocked cross-user writes, but not unvalidated same-user writes). The function also now rejects a `NULL` or non-object `p_patch` (`jsonb_typeof(p_patch) != 'object'`) and pins `search_path = public`. This does not reintroduce per-key validation inside the function — that's still intentionally the route's job — it only restricts *who* can call it at all.
