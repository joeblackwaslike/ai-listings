  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      const { data } = await supabase
        .from('listings')
        .select('id, sku, status, title, brand, category, condition, condition_notes, intake_meta, suggested_price_cents, final_price_cents, sold_price_cents, agent_blocked, agent_blocked_reason, pipeline_step, pipeline_total, skip_background_removal')
        .in('id', activeIds)
      if (data) {
        setListings((prev) => {
          let next = prev
