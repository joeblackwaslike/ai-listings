...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Previously uncaught entirely -- any throw here (a Supabase network error, a DB error
    // from getSetting/createOauthState) fell through to Next.js's default error handling with
    // no auth-specific log line at all.
    console.error('[oauth/connect] unexpected error:', err instanceof Error ? err.message : err)
