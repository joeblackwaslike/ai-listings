...
export const backfillEbayDescriptions = inngest.createFunction(
	...
	async ({ step }) => {
		await step.run("backfill", async () => {
			...
			console.log(
				`[backfill-ebay-descriptions] users with eBay creds: ${userIds.length}`,
			);
			...
			for (const userId of userIds) {
				try {
					...
					if (!listings || listings.length === 0) {
						console.log(
							`[backfill-ebay-descriptions] userId=${userId}: no published eBay listings`,
						);
						continue;
					}
					console.log(
						`[backfill-ebay-descriptions] userId=${userId}: updating ${listings.length} listings`,
					);
					...
					for (const listing of listings) {
						const ebayFields = (
							listing.platform_fields as Record<
								string,
						...
						try {
							...
							console.error(
								`[backfill-ebay-descriptions] sku=${sku}: failed`,
								err,
							);
							failed++;
						}
					...
					console.log(
						`[backfill-ebay-descriptions] userId=${userId}: updated=${updated} failed=${failed}`,
					);
				} catch (err) {
					console.error(
						`[backfill-ebay-descriptions] error for userId=${userId}:`,
						err,
					);
				}
			}
