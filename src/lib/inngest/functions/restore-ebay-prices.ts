			...
			for (const userId of userIds) {
				try {
					...
					if (!zeroed || zeroed.length === 0) {
						console.log(
							`[restore-ebay-prices] userId=${userId}: no zeroed listings`,
						);
						continue;
					}
					console.log(
						`[restore-ebay-prices] userId=${userId}: found ${zeroed.length} zeroed listings`,
					);
					...
					console.log(
						`[restore-ebay-prices] userId=${userId}: got prices for ${priceMap.size}/${listingIdToRow.size} listings`,
					);
					...
					for (const [listingId, ebayPrice] of priceMap) {
						...
						console.log(
							`[restore-ebay-prices] restored sku=${row.sku}: 0 → ${ebayPrice}`,
						);
						restored++;
					}
					for (const [listingId] of listingIdToRow) {
						if (!priceMap.has(listingId)) {
							const row = listingIdToRow.get(listingId);
							console.warn(
								`[restore-ebay-prices] sku=${row?.sku}: no price from Browse API`,
							);
							skipped++;
						}
					}
					console.log(
						`[restore-ebay-prices] userId=${userId}: restored=${restored} skipped=${skipped}`,
					);
				} catch (err) {
					console.error(
						`[restore-ebay-prices] error for userId=${userId}:`,
						err,
					);
				}
			}
