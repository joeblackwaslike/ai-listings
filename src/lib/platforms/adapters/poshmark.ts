...
export class PoshmarkAdapter implements PlatformSDK {
  ...
  async replyToOffer(_offerId: string, _action: 'accept' | 'decline' | 'counter', _counterPrice?: number): Promise<void> {
    throw new UnsupportedOperationError(this.platform, 'replyToOffer');
  }
}
